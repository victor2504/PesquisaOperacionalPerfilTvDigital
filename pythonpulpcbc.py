...
def resolver_problema_com_pulp_cbc():
    print("\n--- Solução com Solver CBC ---")
    start = time.time()
    ...
    # Resolver o modelo usando o solver CBC
    ...
    print(f"\n--- Solução (CBC) ---")
    print(f"Status da Solução: {model.status} ({model.solve(PULP_CBC_CMD(msg=False))})") # Status numérico e textual
...
resolver_problema_com_pulp_cbc()